...
mkdir -p -m 755 $CENTOS_ROOT/etc/yum.repos.d
...
cat >$CENTOS_ROOT/etc/yum.repos.d/centos7.repo<<EOF
[centos7]
baseurl=$CENTOS_REPO
enabled=1
gpgcheck=0
...
cat >$CENTOS_ROOT/etc/yum.repos.d/centos7u.repo<<EOF
[centos7u]
baseurl=$CENTOS_REPO_UPD
enabled=1
gpgcheck=0
EOF

yum --installroot=$CENTOS_ROOT --releasever=7 \
    --setopt=override_install_langs=en_US.utf8 --setopt=tsflags=nodocs --setopt=group_package_types=mandatory \
...
sed -i '/distroverpkg=centos-release/a override_install_langs=en_US.utf8\ntsflags=nodocs\ngroup_package_types=mandatory' /etc/yum.conf
echo 'container' > $CENTOS_ROOT/etc/yum/vars/infra
echo 'i386' > $CENTOS_ROOT/etc/yum/vars/basesearch

cat > $CENTOS_ROOT/etc/sysconfig/network <<EOF
NETWORKING=yes
HOSTNAME=localhost.localdomain
EOF

rm -fr $CENTOS_ROOT/etc/yum.repos.d/centos7u.repo $CENTOS_ROOT/etc/yum.repos.d/centos7u.repo
